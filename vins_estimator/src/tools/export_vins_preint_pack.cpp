...
static bool expect_near_abs_rel(const Eigen::MatrixXd &a, const Eigen::MatrixXd &b, double abs_tol, double rel_tol, const std::string &what) {
  if (!a.allFinite() || !b.allFinite()) {
    std::cerr << "[FAIL] " << what << ": non-finite value\n";
    return false;
  }
...
static void write_pack_txt(const std::string &out_txt, const std::string &imu_txt, const std::string &config_yaml, const double ts,
  ...
  ofs << "# jacobian_source: IntegrationBase::jacobian (core normalized midpoint derivatives)\n";
...
static Eigen::Matrix<double, 9, 6> finite_difference_bias(const IntegrationBase &preint, double eps) {
  Eigen::Matrix<double, 9, 6> fd;
  for (int k = 0; k < 6; ++k) {
    Eigen::Vector3d ba_p = preint.linearized_ba, ba_m = ba_p;
    Eigen::Vector3d bg_p = preint.linearized_bg, bg_m = bg_p;
    if (k < 3) { ba_p(k) += eps; ba_m(k) -= eps; }
    else { bg_p(k-3) += eps; bg_m(k-3) -= eps; }

    IntegrationBase preint_p = preint;
    preint_p.repropagate(ba_p, bg_p);
    const Eigen::Vector3d phi_p = so3_log(preint_p.delta_q.toRotationMatrix());

    IntegrationBase preint_m = preint;
    preint_m.repropagate(ba_m, bg_m);
    const Eigen::Vector3d phi_m = so3_log(preint_m.delta_q.toRotationMatrix());

    fd.block<3, 1>(0,k) = (phi_p - phi_m) / (2.0 * eps);
    fd.block<3, 1>(3,k) = (preint_p.delta_p - preint_m.delta_p) / (2.0 * eps);
    fd.block<3, 1>(6,k) = (preint_p.delta_v - preint_m.delta_v) / (2.0 * eps);
  }
  return fd;
}
  ...
  try {
    ...
    for (int i = 1; i < argc; i++) {
      ...
      if (a == "--imu_txt") {
        ...
        out_txt = next();
      } else if (a == "--check_jacobian_fd") {
        check_jacobian_fd = true;
      } else if (a == "--help" || a == "-h") {
        std::cout << "usage: " << argv[0] << " --imu_txt <imu.txt> --config_yaml <config.yaml> --out_txt <out.txt>"
                  << " [--check_jacobian_fd]\n";
        return EXIT_SUCCESS;
      } else {
    ...
    const Eigen::Matrix3d dphi_dbg = T_theta_to_phi * dq_dbg;
    ...
    if (check_jacobian_fd) {
      for (double eps : {1e-5, 1e-6, 1e-7}) {
        const Eigen::Matrix<double, 9, 6> fd = finite_difference_bias(preint, eps);
        std::ostringstream label;
        label << "core JincBias(9x6) vs VINS finite difference, eps=" << std::scientific << eps;
        if (!expect_near_abs_rel(JincBias_ba_bg, fd, 1e-5, 2e-6, label.str())) {
          throw std::runtime_error("internal bias Jacobian failed finite-difference check");
        }
      }
    }
    if (!Sigma_z_gtsam.allFinite() ||
        (Sigma_z_gtsam - Sigma_z_gtsam.transpose()).cwiseAbs().maxCoeff() > 1e-8) {
      throw std::runtime_error("core covariance is non-finite or asymmetric");
    }
    Eigen::SelfAdjointEigenSolver<Eigen::Matrix<double, 15, 15>> eigen(0.5 * (Sigma_z_gtsam + Sigma_z_gtsam.transpose()));
    if (eigen.info() != Eigen::Success || eigen.eigenvalues().minCoeff() < -1e-10) {
      throw std::runtime_error("core covariance is not positive semidefinite");
    }
    std::cout << "[ OK ] core covariance symmetry/PSD\n"
              << "[debug] Jacobian source: IntegrationBase::jacobian\n"
              << "[debug] dphi/dbg (mapped from core):\n" << dphi_dbg << "\n";

    write_pack_txt(out_txt, imu_txt, config_yaml, rows.front().t, rows.back().t, DT, dt_nominal, dR, dP, dV, Sigma_z_gtsam,
                   JincBias_ba_bg);
